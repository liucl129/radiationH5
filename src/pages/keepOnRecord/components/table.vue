	...
	<view class="table-model">
		<uni-table ref="table" :loading="loading" emptyText="暂无更多数据">
			...
			<uni-tr v-for="(ite, j) in table" :key="j">
				<uni-td v-for="(item,index) in title" :key="index">
				{{item.value=="sfkyd"?ite[item.value]==1?'移动':"固定":ite[item.value]}}
				</uni-td>
				<uni-td>
					<view class="uni-warning" @click="del(ite)">
						{{operation}}
					</view>
				</uni-td>
			</uni-tr>
