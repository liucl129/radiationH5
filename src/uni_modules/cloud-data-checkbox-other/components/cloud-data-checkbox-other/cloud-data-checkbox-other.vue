	...
	<view class="uni-data-checklist">
		<template v-if="loading">
			<view class="uni-data-loading">
				<uni-load-more status="loading" iconType="snow" :iconSize="18" :content-text="contentText">
				</uni-load-more>
			</view>
		</template>
		<template v-else>
			<checkbox-group v-if="multiple" class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}"
				@change="chagne">
				<label class="checklist-box" :class="item.labelClass" :style="[item.styleBackgroud]"
					v-for="(item,index) in dataList" :key="index">
					<checkbox class="hidden" hidden :disabled="!!item.disabled" :value="item.value+''"
						:checked="item.selected" />
					<view v-if="(mode !=='tag' && mode !== 'list') || ( mode === 'list' && icon === 'left')"
						class="checkbox__inner" :class="item.checkboxBgClass" :style="[item.styleIcon]">
						<view class="checkbox__inner-icon" :class="item.checkboxClass"></view>
					</view>
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<text class="checklist-text" :class="item.textClass"
							:style="[item.styleIconText]">{{item.text}}</text>
						<view v-if="mode === 'list' && icon === 'right'" class="checkobx__list" :class="item.listClass"
							:style="[item.styleBackgroud]"></view>
					</view>
				</label>
				<view class="" v-if="other">
					<uni-forms-item :name="otherName" label="">
						<uni-easyinput v-model="otherText" :placeholder="otherPlaceholder" @blur="changeOther" />
					</uni-forms-item>
					<!-- <view class="uni-easyinput">
						<view class="uni-easyinput__content is-input-border">
							<input type="text" class="uni-easyinput__content-input" style="padding-left: 10px;" :placeholder="otherPlaceholder"
							 v-model="otherText" @blur="changeOther" />
						</view>
					</view> -->
				</view>
			</checkbox-group>
			<radio-group v-else class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}"
				...
				<label class="checklist-box" :class="item.labelClass" :style="[item.styleBackgroud]"
					...
					<view v-if="(mode !=='tag' && mode !== 'list') || ( mode === 'list' && icon === 'left')"
						class="radio__inner" :class="item.checkboxBgClass" :style="[item.styleBackgroud]">
					...
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<text class="checklist-text" :class="item.textClass"
							:style="[item.styleIconText]">{{item.text}}</text>
						<view v-if="mode === 'list' && icon === 'right'" class="checkobx__list" :class="item.listClass"
							:style="[item.styleRightIcon]"></view>
					</view>
				</label>
				<view class="" v-if="other">
					<uni-forms-item :name="otherName" label="">
...
<script>
	...
	export default {
		...
		watch: {
			...
			otherValue(newVal) {
				//设置其他数据
				this.otherText = newVal;
				console.log("watch otherValue", newVal)
				//this.formItem && this.formItem.setValue(newVal);
			}
		},
		data() {
		...
		created() {
			...
			this.otherText = this.otherValue;
			if (this.formItem) {
				if (this.formItem.name) {
					this.rename = this.formItem.name
					this.otherName = this.formItem.name + "_other";
			...
			if (this.placeholder) {
				this.otherPlaceholder = this.placeholder;
			}
		},
		methods: {
			init(range) {},
			...
			chagne(e) {
				...
				if (this.multiple) {
					...
					hasOther = detail.data.is_other;
				}
				this.formItem && this.formItem.setValue(detail.value)
				console.log("change", detail)

				if (hasOther) {
					console.log("get other", this.otherText)
					this.changeOtherText(this.otherText);
				} else {
					this.changeOtherText(false);
				}
			...
			getDataList(value) {
				...
				dataList.forEach((item, index) => {
					...
					if (item.is_other) {
						if (!this.placeholder) {
							this.otherPlaceholder = "请输入其他" + (item.memo ? item.memo : "");
						}
						if (item.selected) {
							this.other = true;
						}
					}
					if (this.extData && item.ext_fields && item.ext_fields.indexOf(this.extData) > -1) {
						console.log("被排除了", item)
					} else {
						list.push(item)
					}
				})
			...
			changeOtherText(val) {
				if (this.formItem && this.formItem.name) {
					this.form.setValue(this.otherName, val, () => {
						console.log("setvalue ok")
					});
					console.log(this.otherName, val)
